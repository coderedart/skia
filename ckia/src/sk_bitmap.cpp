...
void sk_bitmap_destructor(sk_bitmap_t* cbitmap) { delete AsBitmap(cbitmap); }

sk_bitmap_t* sk_bitmap_new(void) { return ToBitmap(new SkBitmap()); }
...
size_t sk_bitmap_get_row_bytes(sk_bitmap_t* cbitmap) { return AsBitmap(cbitmap)->rowBytes(); }

size_t sk_bitmap_get_byte_count(sk_bitmap_t* cbitmap) {
    return AsBitmap(cbitmap)->computeByteSize();
}

void sk_bitmap_reset(sk_bitmap_t* cbitmap) { AsBitmap(cbitmap)->reset(); }

bool sk_bitmap_is_null(sk_bitmap_t* cbitmap) { return AsBitmap(cbitmap)->isNull(); }

bool sk_bitmap_is_immutable(sk_bitmap_t* cbitmap) { return AsBitmap(cbitmap)->isImmutable(); }

void sk_bitmap_set_immutable(sk_bitmap_t* cbitmap) { AsBitmap(cbitmap)->setImmutable(); }
...
bool sk_bitmap_ready_to_draw(sk_bitmap_t* cbitmap) { return AsBitmap(cbitmap)->readyToDraw(); }
...
bool sk_bitmap_install_pixels(sk_bitmap_t* cbitmap,
                              const sk_imageinfo_t* cinfo,
                              void* pixels,
                              size_t rowBytes,
                              const sk_bitmap_release_proc releaseProc,
                              void* context) {
    return AsBitmap(cbitmap)->installPixels(
            AsImageInfo(cinfo), pixels, rowBytes, releaseProc, context);
}
...
bool sk_bitmap_try_alloc_pixels(sk_bitmap_t* cbitmap,
                                const sk_imageinfo_t* requestedInfo,
                                size_t rowBytes) {
    return AsBitmap(cbitmap)->tryAllocPixels(AsImageInfo(requestedInfo), rowBytes);
}

bool sk_bitmap_try_alloc_pixels_with_flags(sk_bitmap_t* cbitmap,
                                           const sk_imageinfo_t* requestedInfo,
                                           uint32_t flags) {
...
bool sk_bitmap_extract_alpha(sk_bitmap_t* cbitmap,
                             sk_bitmap_t* cdst,
...
sk_shader_t* sk_bitmap_make_shader(sk_bitmap_t* cbitmap,
    ...
    return ToShader(AsBitmap(cbitmap)
                            ->makeShader((SkTileMode)tmx,
                                         (SkTileMode)tmy,
