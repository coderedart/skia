...
size_t sk_codec_min_buffered_bytes_needed(void) { return SkCodec::MinBufferedBytesNeeded(); }
...
sk_codec_t* sk_codec_new_from_stream(sk_stream_t* stream, sk_codec_result_t* result) {
    ...
    return ToCodec(
            SkCodec::MakeFromStream(std::move(skstream), (SkCodec::Result*)result).release());
}
...
void sk_codec_destroy(sk_codec_t* codec) { delete AsCodec(codec); }
...
sk_codec_result_t sk_codec_get_pixels(sk_codec_t* codec,
                                      const sk_imageinfo_t* cinfo,
                                      void* pixels,
                                      size_t rowBytes,
                                      const sk_codec_options_t* coptions) {
    return (sk_codec_result_t)AsCodec(codec)->getPixels(
            AsImageInfo(cinfo), pixels, rowBytes, AsCodecOptions(coptions));
}

sk_codec_result_t sk_codec_start_incremental_decode(sk_codec_t* codec,
                                                    const sk_imageinfo_t* cinfo,
                                                    void* pixels,
                                                    size_t rowBytes,
                                                    const sk_codec_options_t* coptions) {
    return (sk_codec_result_t)AsCodec(codec)->startIncrementalDecode(
            AsImageInfo(cinfo), pixels, rowBytes, AsCodecOptions(coptions));
}
...
sk_codec_result_t sk_codec_start_scanline_decode(sk_codec_t* codec,
                                                 const sk_imageinfo_t* cinfo,
                                                 const sk_codec_options_t* coptions) {
    return (sk_codec_result_t)AsCodec(codec)->startScanlineDecode(AsImageInfo(cinfo),
                                                                  AsCodecOptions(coptions));
}
...
int sk_codec_next_scanline(sk_codec_t* codec) { return AsCodec(codec)->nextScanline(); }
...
void sk_codec_get_frame_info(sk_codec_t* codec, sk_codec_frameinfo_t* frameInfo) {
    ...
    for (size_t i = 0; i < size; i++) cframes[i] = frames[i];
}

bool sk_codec_get_frame_info_for_index(sk_codec_t* codec,
                                       int index,
