...
void sk_canvas_destroy(sk_canvas_t* ccanvas) { delete AsCanvas(ccanvas); }

void sk_canvas_clear(sk_canvas_t* ccanvas, sk_color_t color) { AsCanvas(ccanvas)->clear(color); }

void sk_canvas_clear_color4f(sk_canvas_t* ccanvas, sk_color4f_t color) {
    AsCanvas(ccanvas)->clear(AsColor4f(color));
}

void sk_canvas_discard(sk_canvas_t* ccanvas) { AsCanvas(ccanvas)->discard(); }

int sk_canvas_get_save_count(sk_canvas_t* ccanvas) { return AsCanvas(ccanvas)->getSaveCount(); }
...
void sk_canvas_draw_points(sk_canvas_t* ccanvas,
                           sk_point_mode_t pointMode,
                           size_t count,
                           const sk_point_t points[],
                           const sk_paint_t* cpaint) {
    AsCanvas(ccanvas)->drawPoints(
            (SkCanvas::PointMode)pointMode, count, AsPoint(points), *AsPaint(cpaint));
}
...
void sk_canvas_draw_line(
        sk_canvas_t* ccanvas, float x0, float y0, float x1, float y1, sk_paint_t* cpaint) {
    AsCanvas(ccanvas)->drawLine(x0, y0, x1, y1, *AsPaint(cpaint));
}

void sk_canvas_draw_simple_text(sk_canvas_t* ccanvas,
                                const void* text,
                                size_t byte_length,
                                sk_text_encoding_t encoding,
                                float x,
                                float y,
                                const sk_font_t* cfont,
                                const sk_paint_t* cpaint) {
    AsCanvas(ccanvas)->drawSimpleText(
            text, byte_length, (SkTextEncoding)encoding, x, y, *AsFont(cfont), *AsPaint(cpaint));
}

void sk_canvas_draw_text_blob(
        sk_canvas_t* ccanvas, sk_textblob_t* text, float x, float y, const sk_paint_t* cpaint) {
    AsCanvas(ccanvas)->drawTextBlob(AsTextBlob(text), x, y, *AsPaint(cpaint));
}

void sk_canvas_reset_matrix(sk_canvas_t* ccanvas) { AsCanvas(ccanvas)->resetMatrix(); }
...
void sk_canvas_draw_round_rect(sk_canvas_t* ccanvas,
...
void sk_canvas_clip_rect_with_operation(sk_canvas_t* ccanvas,
                                        const sk_rect_t* crect,
                                        sk_clipop_t op,
                                        bool doAA) {
...
void sk_canvas_clip_path_with_operation(sk_canvas_t* ccanvas,
                                        const sk_path_t* cpath,
                                        sk_clipop_t op,
                                        bool doAA) {
    AsCanvas(ccanvas)->clipPath(*AsPath(cpath), (SkClipOp)op, doAA);
}

void sk_canvas_clip_rrect_with_operation(sk_canvas_t* ccanvas,
                                         const sk_rrect_t* crect,
                                         sk_clipop_t op,
                                         bool doAA) {
...
void sk_canvas_scale(sk_canvas_t* ccanvas, float sx, float sy) { AsCanvas(ccanvas)->scale(sx, sy); }
...
void sk_canvas_skew(sk_canvas_t* ccanvas, float sx, float sy) { AsCanvas(ccanvas)->skew(sx, sy); }
...
void sk_canvas_draw_region(sk_canvas_t* ccanvas,
                           const sk_region_t* cregion,
                           const sk_paint_t* cpaint) {
...
void sk_canvas_draw_circle(
        sk_canvas_t* ccanvas, float cx, float cy, float rad, const sk_paint_t* cpaint) {
...
void sk_canvas_draw_image(sk_canvas_t* ccanvas,
                          const sk_image_t* cimage,
                          float x,
                          float y,
                          const sk_sampling_options_t* sampling,
                          const sk_paint_t* cpaint) {
    AsCanvas(ccanvas)->drawImage(
            AsImage(cimage), x, y, *AsSamplingOptions(sampling), AsPaint(cpaint));
}

void sk_canvas_draw_image_rect(sk_canvas_t* ccanvas,
                               const sk_image_t* cimage,
                               const sk_rect_t* csrcR,
                               const sk_rect_t* cdstR,
                               const sk_sampling_options_t* sampling,
                               const sk_paint_t* cpaint) {
    if (csrcR) {
        AsCanvas(ccanvas)->drawImageRect(AsImage(cimage),
                                         *AsRect(csrcR),
                                         *AsRect(cdstR),
                                         *AsSamplingOptions(sampling),
                                         AsPaint(cpaint),
                                         SkCanvas::SrcRectConstraint::kFast_SrcRectConstraint);
    } else {
        AsCanvas(ccanvas)->drawImageRect(
                AsImage(cimage), *AsRect(cdstR), *AsSamplingOptions(sampling), AsPaint(cpaint));
    }
}

void sk_canvas_draw_picture(sk_canvas_t* ccanvas,
                            const sk_picture_t* cpicture,
...
void sk_canvas_draw_drawable(sk_canvas_t* ccanvas,
                             sk_drawable_t* cdrawable,
                             const sk_matrix_t* cmatrix) {
    SkMatrix m;
    if (cmatrix) {
        m = AsMatrix(cmatrix);
...
void sk_canvas_flush(sk_canvas_t* ccanvas) {
    if (GrDirectContext* dContext = GrAsDirectContext(AsCanvas(ccanvas)->recordingContext())) {
...
sk_canvas_t* sk_canvas_new_from_raster(const sk_imageinfo_t* cinfo,
                                       void* pixels,
                                       size_t rowBytes,
                                       const sk_surfaceprops_t* props) {
    return ToCanvas(
            SkCanvas::MakeRasterDirect(AsImageInfo(cinfo), pixels, rowBytes, AsSurfaceProps(props))
                    .release());
}

void sk_canvas_draw_annotation(sk_canvas_t* t,
...
void sk_canvas_draw_named_destination_annotation(sk_canvas_t* t,
                                                 const sk_point_t* point,
                                                 sk_data_t* value) {
    SkAnnotateNamedDestination(AsCanvas(t), *AsPoint(point), AsData(value));
}

void sk_canvas_draw_link_destination_annotation(sk_canvas_t* t,
                                                const sk_rect_t* rect,
                                                sk_data_t* value) {
...
void sk_canvas_draw_image_lattice(sk_canvas_t* ccanvas,
    ...
    AsCanvas(ccanvas)->drawImageLattice(
            AsImage(image), *AsLattice(lattice), *AsRect(dst), (SkFilterMode)mode, AsPaint(paint));
}
...
void sk_canvas_draw_image_nine(sk_canvas_t* ccanvas,
                               ...
                               sk_filter_mode_t mode,
                               const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawImageNine(
            AsImage(image), *AsIRect(center), *AsRect(dst), (SkFilterMode)mode, AsPaint(paint));
}

void sk_canvas_draw_vertices(sk_canvas_t* ccanvas,
                             const sk_vertices_t* vertices,
                             sk_blendmode_t mode,
                             const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawVertices(AsVertices(vertices), (SkBlendMode)mode, *AsPaint(paint));
}

void sk_canvas_draw_arc(sk_canvas_t* ccanvas,
...
void sk_canvas_draw_drrect(sk_canvas_t* ccanvas,
                           const sk_rrect_t* outer,
                           const sk_rrect_t* inner,
                           const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawDRRect(*AsRRect(outer), *AsRRect(inner), *AsPaint(paint));
}

void sk_canvas_draw_atlas(sk_canvas_t* ccanvas,
                          const sk_image_t* atlas,
                          const sk_rsxform_t* xform,
                          const sk_rect_t* tex,
                          const sk_color_t* colors,
                          int count,
                          sk_blendmode_t mode,
                          const sk_sampling_options_t* sampling,
                          const sk_rect_t* cullRect,
                          const sk_paint_t* paint) {
...
void sk_canvas_draw_patch(sk_canvas_t* ccanvas,
                          const sk_point_t* cubics,
                          const sk_color_t* colors,
                          const sk_point_t* texCoords,
                          sk_blendmode_t mode,
                          const sk_paint_t* paint) {
    AsCanvas(ccanvas)->drawPatch(
            AsPoint(cubics), colors, AsPoint(texCoords), (SkBlendMode)mode, *AsPaint(paint));
}

bool sk_canvas_is_clip_empty(sk_canvas_t* ccanvas) { return AsCanvas(ccanvas)->isClipEmpty(); }

bool sk_canvas_is_clip_rect(sk_canvas_t* ccanvas) { return AsCanvas(ccanvas)->isClipRect(); }

sk_nodraw_canvas_t* sk_nodraw_canvas_new(int width, int height) {
    return ToNoDrawCanvas(new SkNoDrawCanvas(width, height));
}

void sk_nodraw_canvas_destroy(sk_nodraw_canvas_t* t) { delete AsNoDrawCanvas(t); }

sk_nway_canvas_t* sk_nway_canvas_new(int width, int height) {
    return ToNWayCanvas(new SkNWayCanvas(width, height));
}

void sk_nway_canvas_destroy(sk_nway_canvas_t* t) { delete AsNWayCanvas(t); }
...
void sk_nway_canvas_remove_all(sk_nway_canvas_t* t) { AsNWayCanvas(t)->removeAll(); }
...
void sk_overdraw_canvas_destroy(sk_overdraw_canvas_t* canvas) { delete AsOverdrawCanvas(canvas); }
